...
nacc <- fread("gzip -cd ../Data/naccess_human_uniprot.tsv.gz", select=c("ac_uniprot","pos","e_value","acc_s"))
setnames(nacc, "pos", "res")
setkey(nacc,ac_uniprot,res,e_value)[,e_value:=NULL]
setkey(nacc,ac_uniprot,res)
nacc <- unique(nacc, by=c("ac_uniprot","res"))
...
# loading pfam
pfam <- fread("gzip -cd  ../Data/pfam_instances_uniprot.tsv.gz")
